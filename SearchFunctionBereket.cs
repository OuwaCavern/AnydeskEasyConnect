...
{
    internal class SearchFunctionBereket
    {
        internal static void BindSearchData(DataGridView BereketDataGrid)
        {
            BindingSource BereketBindingSource = new()
            {
        ...
        private static List<Anydesks> AramaSonucunuGetir(string subeAdiylaAramaGirdisi, string connectionString)
        {
            ...
            string searchQuery = $"SELECT SubeAdi,SubeAnydeskNumarasi,SubeAnydeskParolasi,BilgisayarYetkisi FROM Bereket WHERE SubeAdi LIKE '%{subeAdiylaAramaGirdisi}%'";
            try
            {
