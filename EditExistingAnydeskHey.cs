...
{
    public partial class EditExistingAnydeskHey : Form
    {
        public EditExistingAnydeskHey()
        {
            InitializeComponent();
        }
        ...
        private void KaydetButonu_Click(object sender, EventArgs e)
        {
            ...
            if (duzenlenenSubeAdi.Count() >= 8)
            {
                ...
                if (eskiAnydeskNumarasi != duzenlenenAnydeskNumarasi)
                {
                    ...
                    string checkAnydeskNoCommand = $"SELECT SubeAnydeskNumarasi,SubeAdi FROM Hey WHERE SubeAnydeskNumarasi='{Algorithms.AnydeskNumarasiDuzenleyici(duzenlenenAnydeskNumarasi)}'";
                    ...
                    {
                        duzenlenenAnydeskNumarasi = Algorithms.AnydeskNumarasiDuzenleyici(duzenlenenAnydeskNumarasi);
                        string updateQuery = $"UPDATE Hey SET SubeAdi='{duzenlenenSubeAdi}',SubeAnydeskNumarasi='{duzenlenenAnydeskNumarasi}',SubeAnydeskParolasi='{duzenlenenAnydeskParolasi}',BilgisayarYetkisi='{duzenlenenBilgisayarYetkisi}' WHERE SubeAnydeskNumarasi='{Algorithms.AnydeskNumarasiDuzenleyici(eskiAnydeskNumarasi)}'";
