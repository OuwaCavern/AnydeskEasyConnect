...
{
    public partial class EditExistingAnydeskHot : Form
    {
        public EditExistingAnydeskHot()
        {
            InitializeComponent();
        }
        ...
        private void KaydetButonu_Click(object sender, EventArgs e)
        {
            ...
            if (duzenlenenSubeAdi.Count() >= 8)
            {
                ...
                if (eskiAnydeskNumarasi != duzenlenenAnydeskNumarasi)
                {
                    ...
                    string checkAnydeskNoCommand = $"SELECT SubeAnydeskNumarasi,SubeAdi FROM Hot WHERE SubeAnydeskNumarasi='{Algorithms.AnydeskNumarasiDuzenleyici(duzenlenenAnydeskNumarasi)}'";
                    ...
                    {
                        string updateQuery = $"UPDATE Hot SET SubeAdi='{duzenlenenSubeAdi}',SubeAnydeskNumarasi='{duzenlenenAnydeskNumarasi}',SubeAnydeskParolasi='{duzenlenenAnydeskParolasi}',BilgisayarYetkisi='{duzenlenenBilgisayarYetkisi}' WHERE SubeAnydeskNumarasi='{Algorithms.AnydeskNumarasiDuzenleyici(eskiAnydeskNumarasi)}'";
