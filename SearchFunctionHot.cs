...
{
    internal class SearchFunctionHot
    {
        internal static void BindSearchData(DataGridView HotDataGrid)
        {
            BindingSource HotBindingSource = new()
            {
        ...
        private static List<Anydesks> AramaSonucunuGetir(string subeAdiylaAramaGirdisi, string connectionString)
        {
            ...
            string searchQuery = $"SELECT SubeAdi,SubeAnydeskNumarasi,SubeAnydeskParolasi,BilgisayarYetkisi FROM Hot WHERE SubeAdi LIKE '%{subeAdiylaAramaGirdisi}%'";
            try
            {
