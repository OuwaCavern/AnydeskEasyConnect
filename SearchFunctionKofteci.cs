...
{
    internal class SearchFunctionKofteci
    {
        internal static void BindSearchData(DataGridView KofteciDataGrid)
        {
            BindingSource KofteciBindingSource = new()
            {
        ...
        private static List<Anydesks> AramaSonucunuGetir(string subeAdiylaAramaGirdisi, string connectionString)
        {
            ...
            string searchQuery = $"SELECT SubeAdi,SubeAnydeskNumarasi,SubeAnydeskParolasi,BilgisayarYetkisi FROM Kofteci WHERE SubeAdi LIKE '%{subeAdiylaAramaGirdisi}%'";
            try
            {
