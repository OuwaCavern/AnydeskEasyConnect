...
{
    internal class SearchFunctionDiger
    {
        internal static void BindSearchData(DataGridView DigerDataGrid)
        {
            BindingSource DigerBindingSource = new()
            {
        ...
        private static List<Anydesks> AramaSonucunuGetir(string subeAdiylaAramaGirdisi, string connectionString)
        {
            ...
            string searchQuery = $"SELECT SubeAdi,SubeAnydeskNumarasi,SubeAnydeskParolasi,BilgisayarYetkisi FROM Diger WHERE SubeAdi LIKE '%{subeAdiylaAramaGirdisi}%'";
            try
            {
