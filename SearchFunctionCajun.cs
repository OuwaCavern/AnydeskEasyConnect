...
{
    internal class SearchFunctionCajun
    {
        internal static void BindSearchData(DataGridView CajunDataGrid)
        {
            BindingSource CajunBindingSource = new()
            {
        ...
        private static List<Anydesks> AramaSonucunuGetir(string subeAdiylaAramaGirdisi, string connectionString)
        {
            ...
            string searchQuery = $"SELECT SubeAdi,SubeAnydeskNumarasi,SubeAnydeskParolasi,BilgisayarYetkisi FROM Cajun WHERE SubeAdi LIKE '%{subeAdiylaAramaGirdisi}%'";
            try
            {
